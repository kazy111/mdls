...
{metadocument}
	<variable>
		...
		<documentation>
			...
			<usage>Check this variable to get the obtained refresh token
				upon successful OAuth authorization.</usage>
		</documentation>
	</variable>
{/metadocument}
...
*/
	...
	Function is_expire($http_code, $error)
	{
		if($http_code === 401 && strpos($error, 'expire') !== false)
			return true;
		else
			return false;
	}

	Function SendAPIRequest($url, $method, $parameters, $oauth, $options, &$response, &$expire)
	{
		...
		{
			$expire = $this->is_expire($http->response_status, $error);
			$http->Close();
	...
	Function CallAPI($url, $method, $parameters, $options, &$response)
	{
		...
		switch(intval($this->oauth_version))
		{
			case 1:
				$url2 = $url;
				$oauth = array(
					$this->token_parameter_name=>$this->access_token
				);
				break;

			case 2:
				$oauth = null;
				$url2 = $url . (strcspn($url, '?') < strlen($url) ? '&' : '?').$this->token_parameter_name.'='.UrlEncode($this->access_token);
				break;
	...
	Function Initialize()
	{
		switch($this->server)
		{
			...
			case 'Bitbucket':
				...
				$this->token_parameter_name = 'oauth_token';
				break;
			...
			case 'Facebook':
				...
				$this->authorization_header = true;
				$this->url_parameters = false;
				$this->token_parameter_name = 'access_token';
				break;
			...
			case 'Google':
				$this->oauth_version = '2.0';
				$this->request_token_url = '';
				$this->dialog_url = 'https://accounts.google.com/o/oauth2/auth?response_type=code&client_id={CLIENT_ID}&redirect_uri={REDIRECT_URI}&scope={SCOPE}&state={STATE}&access_type=offline&approval_prompt=force';
				$this->append_state_to_redirect_uri = '';
			...
			case 'SoundCloud':
				$this->oauth_version = '2.0';
				$this->request_token_url = '';
				$this->dialog_url = 'https://soundcloud.com/connect?client_id={CLIENT_ID}&scope={SCOPE}&response_type=code&redirect_uri={REDIRECT_URI}&state={STATE}';
				$this->append_state_to_redirect_uri = '';
				$this->access_token_url = 'https://api.soundcloud.com/oauth2/token';
				$this->authorization_header = true;
				$this->url_parameters = false;
				$this->token_parameter_name = 'oauth_token';
				break;
			...
			case 'Vimeo':
				$this->oauth_version = '1.0a';
				$this->request_token_url = 'https://vimeo.com/oauth/request_token';
				$this->dialog_url = 'https://vimeo.com/oauth/authorize';
				$this->append_state_to_redirect_uri = '';
				$this->access_token_url = 'https://vimeo.com/oauth/access_token';
				$this->authorization_header = true;
				$this->url_parameters = true;
				$this->token_parameter_name = 'oauth_token';
				break;
		...
		switch(intval($this->oauth_version))
		{
			case 1:
				...
				&& IsSet($access_token['value']))
				{
					...
					|| $expired)
					{
						...
						else
						{
							...
							if(!$this->SendAPIRequest($url, 'GET', array(), $oauth, array('Resource'=>'OAuth access token'), $response, $expire))
								return false;
				...
				if(!IsSet($access_token['authorized']))
				{
					...
					if(!$this->SendAPIRequest($url, 'GET', array(), $oauth, array('Resource'=>'OAuth request token'), $response, $expire))
						return false;
					if(strlen($this->access_token_error))
					{
						$this->authorization_error = $this->access_token_error;
			...
			case 2:
				...
				{
					$this->OutputDebug(serialize($access_token));
				...
				if($state === $stored_state)
				{
					...
					if(!$this->SendAPIRequest($url, 'POST', $values, null, array('Resource'=>'OAuth access token', 'ConvertObjects'=>true), $response, $expire))
						return false;
					if(strlen($this->access_token_error))
					{
						$this->authorization_error = $this->access_token_error;
					...
					if(IsSet($response['refresh_token']))
					{
						...
						$access_token['refresh_token'] = $this->refresh_token;
					}
					else
						$this->refresh_token = '';
...
Function RefreshToken()
{
	...
	if($this->debug)
		$this->OutputDebug('Refreshed access token: '.$this->access_token);
	if(IsSet($response['expires'])
	|| IsSet($response['expires_in']))
	{
		$expires = (IsSet($response['expires']) ? $response['expires'] : $response['expires_in']);
		...
		$this->access_token_expiry = gmstrftime('%Y-%m-%d %H:%M:%S', time() + $expires);
		if($this->debug)
			$this->OutputDebug('Refreshed access token expiry: '.$this->access_token_expiry.' UTC');
		$access_token['expiry'] = $this->access_token_expiry;
	}
	else
		$this->access_token_expiry = '';
	if(IsSet($response['token_type']))
	{
		$this->access_token_type = $response['token_type'];
		if($this->debug)
			$this->OutputDebug('Refreshed access token type: '.$this->access_token_type);
		$access_token['type'] = $this->access_token_type;
	}
	else
		$this->access_token_type = '';
